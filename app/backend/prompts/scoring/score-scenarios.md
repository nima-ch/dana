...
2. **EVIDENCE MAP** — for each scenario: supporting clues (with credibility scores and bias flags), forum arguments for and against, scratchpad intelligence (private strategic intent of parties), and party power projection (net influence weight)
3. **CLUE REGISTRY** — full clue details: type (FACT/NEWS), source, credibility score, bias flags, summary
4. **PARTY REGISTRY** — all parties with weights, ally/rival relationships

## Scoring Rules

**Clue evidence:**
- Weight each clue by its `source_credibility.score` (0–1)
- Discount clues with bias flags: subtract 0.1 per flag, minimum weight 0.1
...
- Disputed or misleading clues (from fact-checker) carry near-zero weight — treat them as intelligence about intent, not as facts
