...
export function dbUpsertParty(topicId: string, party: Party): void {
  getDb().run(
    `INSERT INTO parties (id, topic_id, name, type, description, weight, weight_factors, agenda, means, circle, stance, vulnerabilities, auto_discovered, user_verified)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
     ON CONFLICT(id, topic_id) DO UPDATE SET
       name=excluded.name, type=excluded.type, description=excluded.description,
       weight=excluded.weight, weight_factors=excluded.weight_factors, agenda=excluded.agenda,
       means=excluded.means, circle=excluded.circle, stance=excluded.stance,
    ...
    [party.id, topicId, party.name, party.type, party.description, party.weight,
     JSON.stringify(party.weight_factors), party.agenda, JSON.stringify(party.means),
